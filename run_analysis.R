...
xtest <- read.table("X_test.txt", col.names = variable_names)
ytest <- read.table("y_test.txt")
subject_test <- read.table("subject_test.txt")
        test <- cbind(subject_test, ytest, xtest)

##Train data read into R and subject, x, and y combined
xtrain <- read.table("X_train.txt", col.names = variable_names)
...
dfmelt <- melt(df1, id = c("subject", "activity"), measure.vars = 3:75)
